    ...
    await Promise.all(
      sources.map(async (source) => {
        ...
        try {
          ...
          if (parsed.length === 0) {
            return { source_id: source.id, name: source.name, items: 0 };
          }
          ...
          if (err.error) {
            return { source_id: source.id, name: source.name, items: 0, error: err.error };
          }
          return { source_id: source.id, name: source.name, items: rows.length };
        } catch (e) {
          const message = e instanceof Error ? e.message : "Fetch/parse error";
          return { source_id: source.id, name: source.name, items: 0, error: message };
        }
      })
    )
  ).filter((r): r is { source_id: string; name: string; items: number; error?: string } => Boolean(r));
